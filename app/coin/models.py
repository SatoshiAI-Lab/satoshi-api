...
class Coin(models.Model):
    ...
    def get_chain(self) -> list:
        ...
        """
        chain_obj: models.RawQuerySet[Any] = CoinChainData.objects.using(alias='coin_source').raw(raw_query=chain_sql)
        chain: list = []
        if chain_obj:
            for c in chain_obj:
                chain.append(dict(
                    address=c.address, 
                    name = c.symbol, 
                    logo = urljoin(base=os.getenv('IMAGE_DOMAIN'), url=c.logo) if c.logo else os.getenv(key='IMAGE_DOMAIN') + '/img/chain/base.png',
                ))
        return chain
