...
class CoinCrossQuoteView(APIView):
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:    
        ...
        if status_code != 200 or resp.get('code') != 200:
            return ResponseUtil.custom(status=status_code, code=resp['code'], data=['data'], msg = resp['message'])
...
class CoinCrossStatusView(APIView):
    ...
    def get(self, request) -> Response:
        ...
        if status_code != 200 or resp.get('code') != 200:
            return ResponseUtil.custom(status=status_code, code=resp['code'], data=resp['data'], msg = resp['message'])
        ...
        if 'toTxHash' in status_data:
            ...
            data['to_url'] = constants.CHAIN_DICT[chain]['tx_url'] + status_data['toTxHash']
