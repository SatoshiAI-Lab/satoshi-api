...
class UserSubscription(models.Model):
    ...
    def clean(self):
        ...
        if self.message_type == 0: # news
            ...
            if not (isinstance(self.content, list) and all(isinstance(item, dict) and len(item.get('address', '')) > 40 and item.get('chain', '') in CHAIN_DICT for item in self.content)):
